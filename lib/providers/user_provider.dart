...
class UserProvider extends ChangeNotifier {
  ...
  void _recalculateMatches() {
    // Dedupe by recipe id so the same recipe never appears twice in suggestions
    final seenIds = <String>{};
    final combinedRecipes = <Recipe>[];
    for (final r in [..._allRecipes, ..._remoteRecipes]) {
      if (seenIds.add(r.id)) combinedRecipes.add(r);
    }
  ...
  /// Recipe matches filtered by meal type tag; still ingredient-based, no overlap.
  List<RecipeMatch> getRecipeMatchesForMeal(String mealType) {
    final lower = mealType.toLowerCase();
    return _recipeMatches.where((match) {
      final tags = match.recipe.tags;
      if (tags == null) return false;
      return tags.any((tag) => tag.toLowerCase().contains(lower));
    }).toList();
  }

  /// Unique recipe matches (already deduped by id from _recalculateMatches).
  List<RecipeMatch> get recipeMatchesUnique => _recipeMatches;

  List<Recipe> getOneIngredientAway() {
    final list = _recipeMatches
        .where((match) => match.missingIngredients.length <= 1 && match.matchPercentage >= 70)
